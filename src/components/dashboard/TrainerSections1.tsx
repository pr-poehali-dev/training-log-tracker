...
export function StudentsSection({ user, date, month }: { user: AppUser; date: string; month: string }) {
  ...
  const canEditJournal = user.role === "admin" || user.can_edit_journal !== false;
  // Тренер может отмечать/снимать посещение только за сегодняшний день, админ — за любой
  const canEdit = user.role === "admin" || (canEditJournal && isToday);
  ...
  const openEdit = (s: Record<string, unknown>) => {
    setEditForm({
      ...
      federation_contract: Boolean(s.federation_contract),
    });
    setEditStudent(s);
  };
      ...
      {!canEdit && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-700 font-semibold">
          ...
          {!canEditJournal
            ...
            : "Посещение можно отмечать только за сегодняшний день"}
        </div>
      )}
